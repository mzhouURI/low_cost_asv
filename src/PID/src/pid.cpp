...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  ROS_ASSERT_MSG(setpoint_timeout_ == -1 || setpoint_timeout_ > 0, "setpoint_timeout set to %.2f but needs to -1 or >0",
                 setpoint_timeout_);
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
    ...
    if (pid_enabled_ &&
        (setpoint_timeout_ == -1 || (ros::Time::now() - last_setpoint_msg_time_).toSec() <= setpoint_timeout_))
    {
