...
int main(int argc, char** argv)
{
  ...
  ros::Publisher voltage_pub = vi.advertise<std_msgs::String>("voltage", 1000);
  ros::Publisher current_pub = vi.advertise<std_msgs::String>("current", 1000);
  ...
  while (ros::ok())
  {
	...
	ROS_INFO("voltage0:%d\n voltage1:%d\n", int(vo[0]),int(vo[1]));
/*     std_msgs::String voltage, current;
    std::stringstream s1, s2;
    s1 << atoi(reinterpret_cast<char*>(cu)) * 1.25 << count;
    s2 << atoi(reinterpret_cast<char*>(vo)) * 1.25;
    current.data = s1.str();
    // ss << (int32_t)vo * 1.25;
    voltage.data = s2.str();
    ROS_INFO("current:%s\n", current.data.c_str());
